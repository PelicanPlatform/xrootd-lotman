...
TEST(DirNodeToJsonTest, ConstructsJsonForEmptyDirs) {
    // To test the full functionality of this in a way that also reports directory sizes,
    // I'd need to mock XrdPfc::DataFsPurgeshot, which has a non-overridable function for
    // getting the size of a directory. This also requires real directories. Since I'm not
    // testing the underlying XrdPfc::DataFsPurgeshot functionality, I'll just test the
    // JSON conversion here.

    // DirNode is one of the plugin's structs to act as an intermediary between the DataFsFPurgeshot
    // class, the DirPurgeElement class, and the JSON object that LotMan uses to update lot usage.
    DirNode subNode1;
    ...
    // name of the dir (_not_ the complete path), and which indices in the purge shot's
    // vector of DirPurgeElements constitute child directories.
...
TEST(reconstructPathsAndBuildJson, TypicalCase) {
    // Given a constructed DataFsPurgeshot, reconstruct the paths and build a JSON object.

    // Again, the test won't actually check sizes, because that requires a mocked DataFSPurgeshot,
    // or the construction of a real one that points to real directories.
    XrdPfc::DataFsPurgeshot purge_shot;
    // For some reason, it seems like the root element always has an empty directory name.
    // Letting this creep into the LotUpdate JSON object would throw Lotman off, so the
    // reconstruction function needs to handle this.
    XrdPfc::DirPurgeElement rootElement;
...
TEST(GetPolicyNameTest, ReturnsCorrectPolicyName) {
    EXPECT_EQ(XrdPfc::getPolicyName(XrdPfc::PurgePolicy::PastDel), "LotsPastDel");
    EXPECT_EQ(XrdPfc::getPolicyName(XrdPfc::PurgePolicy::PastExp), "LotsPastExp");
    EXPECT_EQ(XrdPfc::getPolicyName(XrdPfc::PurgePolicy::PastOpp), "LotsPastOpp");
    EXPECT_EQ(XrdPfc::getPolicyName(XrdPfc::PurgePolicy::PastDed), "LotsPastDed");
    EXPECT_EQ(XrdPfc::getPolicyName(XrdPfc::PurgePolicy::UnknownPolicy), "UnknownPolicy");
}

TEST(GetPolicyFromConfigNameTest, ReturnsCorrectPolicyEnum) {
    // These "names" come from the cache configuration. They're abbreviated to match the
    // general "xrootd" style of doing things, so they're not the same as the names returned
    // by getPolicyName.
    EXPECT_EQ(XrdPfc::getPolicyFromConfigName("del"), XrdPfc::PurgePolicy::PastDel);
    EXPECT_EQ(XrdPfc::getPolicyFromConfigName("exp"), XrdPfc::PurgePolicy::PastExp);
    EXPECT_EQ(XrdPfc::getPolicyFromConfigName("opp"), XrdPfc::PurgePolicy::PastOpp);
    EXPECT_EQ(XrdPfc::getPolicyFromConfigName("ded"), XrdPfc::PurgePolicy::PastDed);
    EXPECT_EQ(XrdPfc::getPolicyFromConfigName("foobar"), XrdPfc::PurgePolicy::UnknownPolicy);
    EXPECT_EQ(XrdPfc::getPolicyFromConfigName(""), XrdPfc::PurgePolicy::UnknownPolicy);
}
